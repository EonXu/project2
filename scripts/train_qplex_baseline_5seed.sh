...
# QPLEX 原始 DDFG 默认核心参数：
# weighted_head=True, state_bias=True, is_minus_one=True
# nonlinear 原始默认 False，因此这里不传 --nonlinear
qplex_args="--mixer_hidden_dim 32 --hypernet_hidden_dim 64 --hypernet_layers 2 --n_head 4 --num_kernel 10 --adv_hypernet_embed 64 --adv_hypernet_layers 3 --attend_reg_coef 0.001 --weighted_head --state_bias --is_minus_one"
...
for seed in 1 2 3 4 5; do
  echo "=============================="
  echo "QPLEX pure baseline seed=${seed}"
  echo "=============================="

  # Stage1: 2M steps，对齐 SDDFG Stage1-A1+A2 总步数，但不使用 SDDFG adj schedule
  exp_stage1="qplex_wolfpack_stage1"
  run_stage "stage1" "${exp_stage1}" "qplex_stage1_seed${seed}" "${seed}" "" \
  ...
  exp_stage2a="qplex_wolfpack_stage2a"
  run_stage "stage2a" "${exp_stage2a}" "qplex_stage2a_seed${seed}" "${seed}" "${stage1_model}" \
  ...
  exp_stage2b="qplex_wolfpack_stage2b"
  run_stage "stage2b" "${exp_stage2b}" "qplex_stage2b_seed${seed}" "${seed}" "${stage2a_model}" \
  ...
  exp_stage3="qplex_wolfpack_stage3"
  run_stage "stage3" "${exp_stage3}" "qplex_stage3_seed${seed}" "${seed}" "${stage2b_model}" \
    1500000 0.20 0.20 0.10 0.05 0.05 500000 0 0

  echo "Finished QPLEX seed=${seed}"
done