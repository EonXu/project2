...
# =========================================================
# 计划 2A：Stage3 from run12
# 来源：run12 = 旧 A2 final/models → Stage2
# 目标：
# 1. 使用当前最强 Stage2 作为 Stage3 主线；
# 2. 参考旧版 Stage3 最终压测思想；
# 3. 验证其在 add/del=0.20 极端动态下的最终性能。
# =========================================================
...
algo="sddfg"
exp="sddfg426_stage3"
#name="stage3_from_stage2_run12_final"
#seed=1
#
#
## ==================== 2. 加载 Stage2 run7 final/models ====================
#model_dir="/root/SDDFG/scripts/results/wolfpack/sddfg/sddfg426_stage2/run12/best_models/best_win"
#
#
...
#
#
#echo "seed is ${seed}, running Stage3 from Stage2 run12"
...
name="stage3_from_stage2_run7_best_win_old_settings"
seed=1


# ==================== 2. 加载 Stage2 run7 最优 reward 模型 ====================
model_dir="/root/SDDFG/scripts/results/wolfpack/sddfg/sddfg426_stage2/run7/best_models/best_win"
...
# ==================== 10. 学习率 floor ====================
policy_lr_decay_floor=0
critic_lr_decay_floor=0
...
echo "seed is ${seed}, running Stage3 from Stage2 run7"
echo "model_dir=${model_dir}"
...
CUDA_VISIBLE_DEVICES=0 python train/train_wolfpack.py --env_name ${env} \
  ...
  --sparsity ${sparsity} --gain ${gain} --adj_begin_step ${adj_begin_step} \
  --policy_lr_decay_floor ${policy_lr_decay_floor} --critic_lr_decay_floor ${critic_lr_decay_floor} \
  --use_vfunction --use_wandb --use_linear_lr_decay --use_adj_linear_lr_decay --adj_lr_decay_floor 0.0 --use_dyn_graph \
  --use_reward_normalization --use_valuenorm