...
env="wolfpack"
algo="ddfg"
exp="ddfg-wolfpack-open-close-stage3"
name="open_remove_add_close_penalty"
seed_min=1
seed_max=1
...
# 预训练模型路径 (载入阶段一)
model_dir="/root/SDDFG/scripts/results/wolfpack/ddfg/ddfg-wolfpack-open-close-stage2/run2/models"
...
for seed in $(seq ${seed_min} ${seed_max}); do
  ...
  CUDA_VISIBLE_DEVICES=0 python train/train_wolfpack.py --env_name ${env} \
    ...
    --adj_anneal_time ${adj_anneal_time} --use_vfunction --use_wandb --gamma ${gamma} \
