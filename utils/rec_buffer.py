...
class RecReplayBuffer(object):
    ...
    def insert(self, num_insert_episodes, obs, share_obs, acts, rewards, dones, dones_env, avail_acts, adj=None,prob_adj=None):
        """
    ...
    def norm_reward(self, ind):
        ...
        for p_id in self.policy_info.keys():
            rewards[p_id] = self.policy_buffers[p_id].norm_reward(ind)
...
class RecPolicyBuffer(object):
    ...
    def sample_inds(self, sample_inds):
        ...
        adj = self.adj[:,sample_inds]
        prob_adj = self.prob_adj[:,sample_inds]
        if self.use_avail_acts:
