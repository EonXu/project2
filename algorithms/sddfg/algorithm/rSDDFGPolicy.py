...
class R_SDDFGPolicy(MLPPolicy):
    ...
    def __init__(self, config, policy_config, train=True):
        ...
        self.rnn_network = RNNBase(self.args, self.rnn_network_input_dim, self.rnn_hidden_size, self.rnn_out_dim,
                                   self.device)
        self.rnn_critic_network = RNNBase(self.args, self.central_obs_dim, self.rnn_hidden_size, self.rnn_out_dim,
                                          self.device)

        self.q_network = {
            num_orders: AgentQFunction(self.args, self.rnn_network_input_dim, self.q_network_input_dim, num_orders,
                                       self.q_out_size[num_orders - 1], self.device) for num_orders in
            range(1, self.highest_orders + 1)}

        if self.use_vfunction:
            # self.v_network = AgentVFunction(self.args, self.q_network_input_dim, self.central_obs_dim, 1, 1, self.device)
            self.v_network = {
                num_orders: AgentVFunction(self.args, self.q_network_input_dim, self.central_obs_dim, num_orders, 1,
                                           self.device) for num_orders in range(1, self.highest_orders + 1)}
            self.vtot_network = AgentVFunction(self.args, self.q_network_input_dim, self.central_obs_dim, 1, 1,
                                               self.device)
    ...
    def get_hidden_states(self, obs, prev_actions, rnn_states, dones=None):
        ...
        input_batch = input_batch.to(**self.tpdv)
        rnn_states = to_torch(rnn_states).to(**self.tpdv)

        # 单步：直接 forward
        if input_batch.dim() == 2 or dones is None:
            q_batch, new_rnn_states, no_sequence = self.rnn_network(input_batch, rnn_states)
            if dones is not None:
                d = to_torch(dones).to(new_rnn_states.device).float().view(-1, 1)
                new_rnn_states = new_rnn_states * (1.0 - d)
            return q_batch, new_rnn_states, False

        # 序列：逐步 forward + 按 dones reset hidden
        dones_t = to_torch(dones).to(self.device).float()
        if dones_t.dim() == 4:
            # (T,B,N,1) -> (T, B*N, 1)
            T, B, N, _ = dones_t.shape
            dones_t = dones_t.view(T, B * N, 1)
        elif dones_t.dim() == 3:
            # (T, BN, 1)
            pass
        else:
            raise RuntimeError(f"Unexpected dones shape: {dones_t.shape}")

        outputs = []
        hid = rnn_states
        T = input_batch.shape[0]
        for t in range(T):
            out_t, hid, _ = self.rnn_network(input_batch[t], hid)
            mask = (1.0 - dones_t[t]).to(hid.dtype)  # (BN,1)
            hid = hid * mask
            outputs.append(out_t.squeeze(0) * mask)

        q_batch = torch.stack(outputs, dim=0)  # (T, BN, hidden)
        return q_batch, hid, False

    def get_vtot(self, obs, rnn_states):
        input_batch = to_torch(obs)

        q_batch, new_rnn_states, no_sequence = self.rnn_critic_network(input_batch.to(**self.tpdv),
    ...
    def _filter_alive_idx_node_order(self, idx_node_order, dones):
        """
        删除包含死亡/空槽节点的整条 factor，避免后续降阶或分母污染。
        idx_node_order[i]: [num_edges, 2 + order]
        dones: [B, N, 1]
        """
        if dones is None:
            return idx_node_order

        filtered = []
        dones_t = to_torch(dones).to(self.device).float()

        for order_idx, tmp in enumerate(idx_node_order):
            order = order_idx + 1
            if len(tmp) == 0:
                filtered.append(tmp)
                continue

            b_idx = tmp[:, 0].unsqueeze(1).expand(-1, order)
            a_idx = tmp[:, 2:]
            node_dones = dones_t[b_idx, a_idx]
            keep = (node_dones.max(dim=1)[0].reshape(-1) < 0.5)

            filtered.append(tmp[keep])
    ...
    def get_v_batch(self, obs_batch, state_batch, adj_input=None, batch_size=1, no_sequence=False, dones=None):
        ...
        for i in range(1, self.highest_orders + 1):
            ...
            tmp = torch.cat([idx[0].unsqueeze(-1), idx[1].unsqueeze(-1), torch.where(adj_input[idx])[1].reshape(-1, i)],
                            dim=-1)
            idx_node_order.append(tmp)

        # 新增：在进入 V 网络前删除含死亡节点 factor
        idx_node_order = self._filter_alive_idx_node_order(idx_node_order, dones)
        ...
        for i in range(self.highest_orders):
            tmp = idx_node_order[i]
            if len(tmp) != 0:
                v_out = self.v_network[i + 1](list_obs_batch[i], list_state_batch[i], no_sequence)

                #q_batch.append(self.v_network[i + 1](list_obs_batch[i], list_state_batch[i], no_sequence))
                # ====== 核心修复 1B：切断空槽位的 V-value 污染 ======
                if dones is not None:
                    b_idx = tmp[:, 0].unsqueeze(1).expand(-1, i + 1)
                    a_idx = tmp[:, 2:]
                    node_dones = dones[b_idx, a_idx]
                    edge_dones = node_dones.max(dim=1)[0].float()
                    v_out = v_out * (1.0 - edge_dones)
                # ================================================

                q_batch.append(v_out)
            else:
                q_batch.append([])
    ...
    def get_v_values(self, obs_batch, state_batch, adj_input=None, no_sequence=False, dones=None):
        ...
        v_batch, idx_node_order, adj, num_edges = self.get_v_batch(obs_batch, to_torch(state_batch).to(**self.tpdv),
                                                                   adj_input, batch_size, no_sequence, dones)  # 0.00358
    ...
    def get_rnn_batch(self, obs_batch, rnn_states_batch, adj_input=None, batch_size=1, no_sequence=False, dones=None):
        ...
        # ===== 修改点 3：rSDDFGPolicy.py::get_rnn_batch，idx_node_order 构造完成后插入 =====
        idx_node_order = self._filter_alive_idx_node_order(idx_node_order, dones)
        ...
        for i in range(self.highest_orders):
            tmp = idx_node_order[i]
            if len(tmp) != 0:
                q_out = self.q_network[i + 1](list_obs_batch[i], list_rnn_batch[i], no_sequence)
                #q_batch.append(self.q_network[i + 1](list_obs_batch[i], list_rnn_batch[i], no_sequence))
                # ====== 核心修复 1A：切断空槽位的 Q-value 污染 ======
                if dones is not None:
                    # 获取该条因子边涉及的所有节点的 done 状态
                    b_idx = tmp[:, 0].unsqueeze(1).expand(-1, i + 1)
                    a_idx = tmp[:, 2:]
                    node_dones = dones[b_idx, a_idx]  # [num_edges, i+1, 1]
                    edge_dones = node_dones.max(dim=1)[0].float()  # [num_edges, 1]
                    q_out = q_out * (1.0 - edge_dones)  # 只要有一个节点 dead，整条边失效
                # ================================================
    ...
    def get_q_values(self, obs_batch, rnn_q_states_batch, action_batch, adj_input=None, no_sequence=False, dones=None):
        ...
        q_batch, idx_node_order, adj, num_edges = self.get_rnn_batch(obs_batch.to(**self.tpdv),
                                                                     ...
                                                                     batch_size, no_sequence, dones)  # 0.00358
    ...
    def q_values(self, f_q, actions, idx_node_order, batch_size):
        ...
        if len(idx_node_order[0]) != 0:
            ...
            values[0] = scatter_add(src=tmp1, index=index1, dim=0)[:-1].reshape(-1, batch_size)
    ...
    def q_local_values(self, f_q, actions, idx_node_order):
        ...
        values_f = torch.zeros((1, num_f, 1)).to(self.device)
        if len(idx_node_order[0]) != 0:
            edge_actions_1 = (actions[idx_a_of_Q[0][0], idx_a_of_Q[0][1]] * self.act_dim + actions[
    ...
    def greedy(self, adj, q_batch, idx_node_order, available_actions, num_edges, batch_size):
        ...
        if num_edges > 0 and self.args.msg_iterations > 0:
            ...
            for iteration in range(self.args.msg_iterations):
                ...
                utils_a = avail_a + scatter_add(
                    src=messages_Q2a,
                    index=var_edge,
                    dim=0,
                    dim_size=batch_size * self.n_agents
                ).reshape(batch_size, self.n_agents, self.act_dim)
                ...
                utils_Q = scatter_add(
                    ...
                    index=f_edge,
                    dim=0,
                    dim_size=batch_size * self.num_factor
                ).reshape(batch_size, self.num_factor, self.act_dim, self.act_dim, self.act_dim)
    ...
    def get_actions(self, obs_batch, rnn_q_states_batch, available_actions=None, t_env=None, explore=False,
        ...
        if self.multidiscrete:
            ...
            # ===== 修改点 1：替换 get_actions 中 else: if explore: 分支 =====
    ...
    # ===== 修改点 2：替换 get_random_actions 整个函数 =====
