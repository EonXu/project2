...
from algorithms.qplex.algorithm.dmaq_general import DMAQer
from algorithms.qplex.algorithm.dmaq_qatten import DMAQ_QattenMixer
...
class QPlex(Trainer):
    ...
    def train_policy_on_batch(self, batch, update_policy_id=None):
        ...
        for p_id in self.policy_ids:
            ...
            pol_obs_batch = to_torch(obs_batch[p_id]).to(**self.tpdv)
            curr_act_batch = to_torch(act_batch[p_id]).to(**self.tpdv)

            # pol_obs_batch: [n_agents, T+1, B, obs_dim]
            # inactive slots are padded by all -1 in the Wolfpack wrapper.
            # QPLEX mixer uses [T, B, n_agents, ...], so keep a final singleton dim.
            curr_alive = (pol_obs_batch[:, :-1] != -1.0).any(dim=-1).float().permute(1, 2, 0).unsqueeze(-1)
            next_alive = (pol_obs_batch[:, 1:] != -1.0).any(dim=-1).float().permute(1, 2, 0).unsqueeze(-1)
            ...
            _, max_action_qvals = policy.actions_from_q(
                mac_out[:-1],
                available_actions=avail_actions[:-1]
            )
            cur_max_actions, _ = policy.actions_from_q(
                mac_out[1:],
                available_actions=avail_actions[1:]
            )
            cur_max_actions_onehot = torch.tensor(cur_max_actions, dtype=torch.float32).to(**self.tpdv)

            _, target_max_qvals = target_policy.actions_from_q(
                target_mac_out[1:],
                available_actions=avail_actions[1:]
            )
            ...
            curr_alive = curr_alive.to(device=chosen_action_qvals.device, dtype=chosen_action_qvals.dtype)
            next_alive = next_alive.to(device=target_chosen_qvals.device, dtype=target_chosen_qvals.dtype)
            # Dynamic-slot correction:
            # inactive agents must not affect QPLEX value/advantage mixing.
            actions_onehot = actions_onehot * curr_alive
            cur_max_actions_onehot = cur_max_actions_onehot * next_alive

            chosen_action_qvals = chosen_action_qvals * curr_alive
            max_action_qvals = max_action_qvals * curr_alive

            target_chosen_qvals = target_chosen_qvals * next_alive
            target_max_qvals = target_max_qvals * next_alive
