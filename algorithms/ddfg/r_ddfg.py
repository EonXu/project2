...
class R_DDFG:
    ...
    def train_policy_on_batch(self, batch, use_same_share_obs=None):
        ...
        for p_id in self.policy_ids:
            ...
            alive_mask = (1.0 - dones.float()).clamp(0.0, 1.0)  # [step, batch, N, 1]
            ...
            # 死亡/空槽 agent 的 self-factor 清零
            eye = eye * alive_mask.long()

            if self.use_dyn_graph:
                adj_dyn = adj.to(self.device).long()  # [step, batch, N, F]

                # factor-level mask：含任一死亡节点的动态 factor 整条删除
    ...
    def train_adj_on_batch(self, batch, use_adj_init, use_same_share_obs=None):
        ...
        # 同理，增加截断
        prob_adj_safe = torch.where(adj == 1, prob_adj, torch.ones_like(prob_adj, dtype=torch.float32)).clamp_min(1e-8)
        log_prob_adj = torch.log(prob_adj_safe)

        # ===== 修改点：替换 active_masks 到 ratio 计算前的 mask 逻辑 =====
        ...
        # f_advts: [B, F, 1]，必须同步删除非法 factor 的优势，避免非法 factor 产生梯度
        f_advts = f_advts * valid_factor_masks.transpose(1, 2)

        # 后续 order 判定也必须基于有效 adj，而不是原始 adj
        ...
        if self.highest_orders == 3:
            ...
            imp_weights_multinomial = torch.where(valid_adj.sum(-2) == 1, imp_weights * imp_weights, imp_weights).unsqueeze(
                -1)
        else:
            diff_log = torch.clamp(tarlog_prob_adj.sum(-2) - log_prob_adj.sum(-2), max=80)
            imp_weights = torch.exp(diff_log)
        ...
        rl_loss = -(torch.sum(torch.min(surr1, surr2), dim=-2) * (1 - bad_transitions_mask)).sum() / (
                (1 - bad_transitions_mask).sum() + 1e-3)
