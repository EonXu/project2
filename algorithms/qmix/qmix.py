...
class QMix(Trainer):
    ...
    def train_policy_on_batch(self, batch, update_policy_id=None):
        ...
        # individual agent q value sequences: each element is of shape (ep_len, batch_size, n_agents)
        agent_q_seq = []
        # individual agent next step q value sequences
        agent_nq_seq = []
        # current / next active masks inferred from padded observations
        agent_alive_seq = []
        agent_next_alive_seq = []
        batch_size = None
        ...
        for p_id in self.policy_ids:
            ...
            agent_next_alive_seq.append(next_alive)
        ...
        alive_seq = torch.cat(agent_alive_seq, dim=-1).to(device=agent_q_seq.device, dtype=agent_q_seq.dtype)
        next_alive_seq = torch.cat(agent_next_alive_seq, dim=-1).to(device=agent_nq_seq.device,
                                                                    dtype=agent_nq_seq.dtype)

        # Dynamic-slot correction:
        # inactive agents must not contribute to VDN sum or QMIX mixer input.
        agent_q_seq = agent_q_seq * alive_seq
        agent_nq_seq = agent_nq_seq * next_alive_seq
